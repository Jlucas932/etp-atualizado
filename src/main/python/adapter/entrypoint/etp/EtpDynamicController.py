    ...
    try:
        ...
        if current_stage == 'collect_need':
            ...
            if is_free_confirmation:
                ...
                # Build response from generator output with intro fallback
                intro = (result.get('intro') or '').strip()
                if not intro:
                    intro = "Nota técnica inicial: a lista a seguir foi ajustada conforme seu retorno, mantendo foco em critérios verificáveis, SLAs e aderência regulatória."
                if new_requirements:
...
Motivo técnico: permite prosseguir sem aguardar próximo ciclo anual, conforme urgência da demanda."""
...
Motivo técnico: asseguram conformidade licitatória e regulatória mínima."""
...
Motivo técnico: base prática para revisão posterior sem travar o ETP."""
...
Motivo técnico: equilibra competitividade e controle de entrega. {recommendation}"""
...
Posso propor uma diretriz de parcelamento e você decide: **aceitar**, **marcar pendente** ou **debater**?"""
                    ...
                    if 'opt1' in user_lower or 'sem parcelamento' in user_lower:
                        answers['installment'] = 'Não haverá parcelamento. Motivo técnico: busca de economia de escala e uniformidade da solução.'
                    elif 'opt2' in user_lower or 'com parcelamento' in user_lower:
                        answers['installment'] = 'Haverá parcelamento em lotes. Motivo técnico: ampliar competitividade e permitir participação de ME/EPP.'
                    elif 'opt3' in user_lower or 'híbrido' in user_lower:
                        answers['installment'] = 'Haverá parcelamento híbrido. Motivo técnico: equilibrar economia de escala com competitividade.'
                    else:
...
def preview_etp_document(session_id):
    ...
    try:
        ...
        # Load EtpParts from conversation metadata
        # Note: session_id might be used as conversation_id in some contexts
        parts = load_parts(session_id)
        
        # Assemble document sections using the Assembler
        from application.etp.types import DocSection
        assembled = assemble_sections(parts)
        
        # Convert DocSection enums to string keys for JSON response
        sections = {
            'titulo': f"Estudo Técnico Preliminar - {parts.necessidade_texto or 'Contratação'}",
            'introducao': assembled.get(DocSection.INTRODUCAO, ''),
            'necessidade': assembled.get(DocSection.OBJETO_DESC_NECESSIDADE, parts.necessidade_texto or "Não informada"),
            'requisitos': parts.requisitos or [],
            ...
            'solucao_como_um_todo': assembled.get(DocSection.SOLUCAO_COMO_UM_TODO, ''),
            'parcelamento': assembled.get(DocSection.JUSTIFICATIVA_PARCELAMENTO, ''),
            'sumario': parts.resumo_executivo or 'Aguardando finalização do fluxo'
        }
