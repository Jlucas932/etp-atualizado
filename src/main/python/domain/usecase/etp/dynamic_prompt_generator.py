...
class DynamicPromptGenerator:
    ...
    def _clean_and_format_rag_content(self, content: str) -> str:
        """Limpa e formata conteúdo extraído do RAG"""
        ...
        try:
            ...
            - Formate como requisito específico
            - Não invente informações que não estão no trecho
            - Retorne apenas o requisito reformulado, sem explicações adicionais
            """
    ...
    def _generate_contextual_suggestions(self, contracting_need: str, contract_type: str) -> tuple:
        ...
        try:
            # Prompt consultivo contextualizado
            consultive_prompt = f"""
            Como consultor especializado em contratações públicas, analise a seguinte necessidade e sugira requisitos específicos e justificados:
            
            Necessidade: "{contracting_need}"
            Tipo: {contract_type}
            
            IMPORTANTE:
            - Sugira apenas requisitos específicos e pertinentes ao objeto descrito
            - Evite requisitos genéricos que se aplicam a qualquer contratação
            - Cada requisito deve vir acompanhado de uma breve justificativa
            - Varie as sugestões conforme o contexto específico
            - Não use sempre os mesmos padrões (anos de experiência, ISO, etc.)
            
            Retorne um JSON com:
            - "requirements": array com 4-6 requisitos específicos e contextualizados
            - "justifications": array com justificativas correspondentes a cada requisito
            - "consultative_message": mensagem explicativa sobre as recomendações
            """
            ...
            # Limpar resposta JSON da OpenAI
            cleaned_json = self._clean_json_response(result_raw)
            
            try:
                result = json.loads(cleaned_json)
            except json.JSONDecodeError as json_error:
                logger.warning(f"[RAG] Resposta JSON inválida, usando fallback de texto cru: {json_error}")
                # Fallback: usar conteúdo bruto como mensagem
                return [
                    f"Requisitos sugeridos para {contracting_need.lower()}",
                    "Especificações técnicas adequadas ao objeto da contratação",
                    "Comprovação de capacidade técnica e operacional",
                    "Garantias adequadas à natureza e valor da contratação"
                ], f"Não foi possível processar as sugestões automaticamente. Conteúdo bruto: {result_raw[:200]}..."
            
            # Combinar requisitos com justificativas
            requirements = result.get('requirements', [])
            justifications = result.get('justifications', [])
            
            # Formatar requisitos com justificativas
            formatted_requirements = []
            for i, req in enumerate(requirements):
                justification = justifications[i] if i < len(justifications) else ""
                if justification:
                    formatted_req = f"{req} (Justificativa: {justification})"
                else:
                    formatted_req = req
                formatted_requirements.append(formatted_req)
            
            consultative_message = result.get('consultative_message', 
                "Como não encontrei informações específicas na base de conhecimento, seguem sugestões contextualizadas baseadas nas melhores práticas:")
                
            return formatted_requirements, consultative_message
            
        except Exception as e:
            logger.error(f"Erro ao gerar sugestões consultivas: {e}")
            # Fallback ultra-simples
            return [
                f"Requisitos técnicos específicos para {contracting_need.lower()}",
                "Comprovação de capacidade técnica adequada ao objeto",
                "Garantia compatível com a natureza do objeto",
                "Prazo de entrega/execução adequado às necessidades"
