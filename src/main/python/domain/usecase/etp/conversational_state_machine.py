...
def parse_user_intent(user_message: str, current_state: str, session_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # [INTENT-GUARD] Ignore confirmation in collect_need stage
    if current_state == 'collect_need' and is_generic_confirmation(user_message):
        import logging
        logger = logging.getLogger(__name__)
        logger.warning("[INTENT-GUARD] ignorando confirmação em collect_need")
        return {
            'intent': 'user_input',
...
def normalize_text(text: str) -> str:
    ...
    Used for more flexible intent matching.
    """
...
def classify_confirmation_intent(user_message: str) -> str:
    """
    Classify user message into three intent classes for confirmation flow:
    - 'positive': User confirms/agrees/wants to proceed
    - 'negative': User disagrees/wants to adjust/refine
    - 'uncertain': User is unsure/needs clarification
    ...
    # POSITIVE patterns - confirmation and agreement
    positive_patterns = [
        r'\bok\b', r'\bokay\b',
        r'\bsim\b', r'\bs\b',
        r'\bclaro\b',
        r'\bpode\b', r'\bpode seguir\b', r'\bpode prosseguir\b',
        r'\bconfirm\w*\b',  # confirme, confirmo, confirmado, confirmação
        r'\bconcord\w*\b',  # concordo, concordar
        r'\baceit\w*\b',  # aceito, aceitar, aceitável
        r'\baprov\w*\b',  # aprovado, aprovar, aprovação
        r'\best[aá] (certo|bom|ok|perfeito|correto)\b',
        r'\bt[aá] (bom|certo|ok|perfeito)\b',
        r'\bficou (bom|certo|perfeito|ok)\b',
        r'\bprosseguir\b', r'\bprossiga\b',
        r'\bseguir\b', r'\bsiga\b', r'\bsegue\b',
        r'\bmanter\b', r'\bmant[ée]m\b',
        r'\bfechou\b',
        r'\bperfeito\b', r'\b[oo]timo\b', r'\bexcelente\b',
        r'\bcorreto\b', r'\bcerto\b',
        r'\bbeleza\b', r'\bshow\b',
        r'\bmanda (bala|brasa|ver)\b',
        r'\bvai fundo\b', r'\bvamos l[aá]\b',
        r'\bposso seguir\b', r'\bposso prosseguir\b'
    ]
    
    # NEGATIVE patterns - disagreement and adjustment requests
    negative_patterns = [
        r'\bn[aã]o\b', r'\bneg\w*\b',
        r'\bdiscord\w*\b',  # discordo, discordar
        r'\bajust\w*\b',  # ajustar, ajuste, ajustes
        r'\brefaz\w*\b', r'\brefin\w*\b',  # refazer, refinar
        r'\bcorrig\w*\b',  # corrigir, correção, corrija
        r'\bmudar\b', r'\bmud[ae]\b', r'\balterar\b',
        r'\bn[aã]o gostei\b', r'\bn[aã]o ficou bom\b',
        r'\bvoltar\b', r'\bretornar\b',
        r'\brefazer\b', r'\brecomeçar\b',
        r'\berr\w*\b',  # erro, errado, errada
        r'\bincorret\w*\b',  # incorreto, incorreta
        r'\bdiferent\w*\b',  # diferente, diferentemente
        r'\bmudança\b', r'\bmodific\w*\b',  # modificar, modificação
        r'\brevisa[ro]\b', r'\brevis[aã]o\b'
    ]
    
    # UNCERTAIN patterns - doubt and need for clarification
    uncertain_patterns = [
        r'\btalvez\b', r'\bpode ser\b',
        r'\bd[uú]vida\b', r'\bestou em d[uú]vida\b',
        r'\bn[aã]o sei\b', r'\bn[aã]o tenho certeza\b',
        r'\bmais detalhes\b', r'\bdetalhar\b',
        r'\bexplicar\b', r'\bexplica\b', r'\bexplique\b',
        r'\bcomo assim\b', r'\bcomo [ée]\b',
        r'\bo que\b.*\?', r'\bqual\b.*\?', r'\bpor que\b.*\?',
        r'\besclare[cç]\w*\b',  # esclarecer, esclarecimento
        r'\bn[aã]o entendi\b', r'\bn[aã]o compreendi\b',
        r'\bpergunt\w*\b',  # pergunta, perguntar
        r'\bme ajud\w*\b',  # me ajude, me ajudar
        r'\binforma[cç]\w*\b'  # informação, informações
    ]
    
    # Check patterns in both original and normalized text
    for pattern in positive_patterns:
        if re.search(pattern, msg_lower) or re.search(pattern, msg_normalized):
            return 'positive'
    ...
    # Default: if message is very short (< 5 chars), treat as uncertain
    # Otherwise treat as positive (benefit of the doubt for natural conversation)
    if len(msg_lower) < 5:
        return 'uncertain'
...
def is_generic_confirmation(user_message: str) -> bool:
    """Check for generic confirmation patterns (backward compatibility)."""
    intent = classify_confirmation_intent(user_message)
